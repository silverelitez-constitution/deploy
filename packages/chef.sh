...
if [[ "$(hostname | cut -d'.' -f1)" != "${svc}" ]]; then echo "Can only be deployed to the ${svc} server. Aborting.."; exit; fi
...
P_INSTALL https://packages.chef.io/files/current/chef-server/12.17.39+20180322095710/el/7/chef-server-core-12.17.39+20180322095710-1.el7.x86_64.rpm
...
service ${svc} restart || { echo "Error. Aborting!"; exit 1; }
