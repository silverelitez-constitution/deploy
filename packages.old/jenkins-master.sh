...
PACKAGE_init() { echo init;
	...
	giturl="https://raw.githubusercontent.com/silverelitez-${realm}/deploy/${branch}/scripts/profile.d/functions-${realm}.sh"
...
PACKAGE_install() { echo install;
	...
	for package in ${software}; do
		sudo yum install -y "${package}" || sleep 10
	done
}
...
PACKAGE_tail() { echo tail; 
	ip addr list dev "$(route | grep '^default' | grep -o '[^ ]*$'| head -n1)" | grep 'inet '
	while ! cat /var/lib/jenkins/secrets/initialAdminPassword; do sleep 1; done
}
