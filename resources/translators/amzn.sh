...
P_NAME() { repoquery --plugins -C --qf %{NAME} --whatprovides *bin/${1}; }
...
P_UPDATE() { sudo yum update -y --skip-broken; sudo yum makecache; }
P_UPGRADE() { sudo yum upgrade -y --skip-broken; sudo yum makecache; }
P_SEARCH() { yum search "${@}"; }
PG_BASH_COMPLETION() {
