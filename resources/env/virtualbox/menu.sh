...
services() {
	...
	SERVICE=$(
		...
		--menu "Please choose a service to:" 0 0 0 \
...
#	terraform init && terraform apply -auto-approve || exit 1
	...
	deployer provisioner $(cat ~/pw) ${ip} 2>&1 | dialog --progressbox "Provisioning server..." 15 80

	dialog --infobox "Installing service..." 0 0
	deployer ${svc} $(cat ~/pw) ${ip} 2>&1 | dialog --progressbox "Installing services..." 15 80
